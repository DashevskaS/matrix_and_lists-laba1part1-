...
public:
    ...
    struct sparse& s_get_link(int index);
    struct matrix& m_get_link(int index);
...
template<typename T>
struct sparse& List<T>::s_get_link(int index)
{
    Node<T>* Cur = Head;
    for (int i = 0; i < index; i++) {
        Cur = Cur->Next;
...
template<typename T>
struct matrix& List<T>::m_get_link(int index)
{
    Node<T>* Cur = Head;
    for (int i = 0; i < index; i++) {
        Cur = Cur->Next;
